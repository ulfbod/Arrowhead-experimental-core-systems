#!/usr/bin/env python3
"""
uncertainty_sim_large_text_v01.py — Uncertainty-Aware Selection Simulation
                                    (large-text / taller variant)
===========================================================================
...
Key differences from uncertainty_sim.py:
  • Figure width  : 3.5 in  →  6.5 in  (wider canvas gives more room)
  • Figure heights : proportionally taller to accommodate larger text
  • FS_TICK       : 8  →  11
  • FS_LABEL      : 9  →  13
  • FS_TITLE      : 9  →  13
  • FS_LEGEND     : 7.5 → 11
...
See uncertainty_sim_large_text_v02.py for a variant that keeps the same
panel heights as uncertainty_sim.py while still using the larger fonts.
...
# Large-text figure dimensions — wider than the IEEE single-column variant so
# that the bigger fonts fit without crowding the plot area.
COL_W  = 6.5    # inches — wide enough for large fonts to breathe
ROW_H  = 3.0    # inches — taller panels to match the increased font height
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--figures-dir", type=Path,
                   default=Path("docs/figures/uncertainty_simulation_large_text"),
...
def main() -> None:
    ...
    print(f"  Reproduce: python scripts/uncertainty_sim_large_text_v01.py "
