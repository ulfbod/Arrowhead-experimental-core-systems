#!/usr/bin/env python3
"""
uncertainty_sim_large_text_v02.py — Uncertainty-Aware Selection Simulation
                                    (large-text / same-height variant)
===========================================================================

Identical simulation logic to uncertainty_sim.py.
Only the figure style differs: fonts are scaled up relative to the plot area
so that legends, axis labels, tick values, and titles are clearly readable
when figures are viewed at screen resolution or inserted into reports/slides.

Key differences from uncertainty_sim.py:
  • Figure width  : 3.5 in  →  6.5 in  (wider canvas gives more room for larger fonts)
  • Figure heights : unchanged — same as uncertainty_sim.py (ROW_H = 2.4 in)
  • FS_TICK       : 8  →  11
  • FS_LABEL      : 9  →  13
  • FS_TITLE      : 9  →  13
  • FS_LEGEND     : 7.5 → 11
  • FS_ANNOT      : 7  →  9
  • LW / MS       : slightly heavier to match the larger text

Difference from uncertainty_sim_large_text_v01.py:
  • v01 also increases panel heights (ROW_H 2.4 → 3.0); v02 keeps ROW_H = 2.4
    so the output figures are about as tall as those from uncertainty_sim.py
    while still carrying the larger, more legible typography.
...
Output layout:
  ...
  docs/figures/uncertainty_simulation_large_text_v02/scenario_{1,2,3}/
...
Usage:
  python scripts/uncertainty_sim_large_text_v02.py
  python scripts/uncertainty_sim_large_text_v02.py --scenario 2 --runs 50 --seed 42
  python scripts/uncertainty_sim_large_text_v02.py --plot-only --scenario all
  python scripts/uncertainty_sim_large_text_v02.py --output-dir results --figures-dir docs/figures
"""
...
# ── Plotting ──────────────────────────────────────────────────────────────────
#
# Figure width is increased to 6.5 in (vs 3.5 in for the IEEE single-column
# variant) so that the larger fonts have room to breathe without crowding the
# plot area. Panel heights are kept identical to uncertainty_sim.py (ROW_H=2.4)
# so the overall figure height matches the original script.
...
# Large-text figure dimensions — wider canvas, same panel heights as uncertainty_sim.py.
COL_W  = 6.5    # inches — wide enough for large fonts to breathe
ROW_H  = 2.4    # inches — same as uncertainty_sim.py (unchanged from base script)
FIG_DPI = 300   # 300 dpi for print/screen quality

# Typography — significantly larger than the IEEE column variant so that every
# text element (ticks, labels, titles, legends) is clearly readable.
FS_TICK    = 11   # axis tick labels
FS_LABEL   = 13   # axis labels (xlabel / ylabel)
FS_TITLE   = 13   # subplot titles
FS_LEGEND  = 11   # legend entries
FS_ANNOT   = 9    # minor annotations (bar value labels, etc.)

# Line and marker weights — heavier to stay visually balanced with larger text
LW        = 2.0   # main median curve
LW_BAND   = 1.0   # p10/p90 dashed lines
LW_GRID   = 0.5
BAND_ALPHA = 0.15
MARKER_EVERY = 10
MS = 6            # marker size
...
def _generate_plots(data_dir: Path, fig_dir: Path, scenario_key: str, cfg: dict) -> None:
    ...
    fig, ax = plt.subplots(figsize=(COL_W, ROW_H * 1.7))
    for pol in POLICIES:
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(COL_W, ROW_H * 2.4), sharex=True,
                                    gridspec_kw={"hspace": 0.35})
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=Path("results/uncertainty_simulation"),
                   help="Root directory for CSV output (shared with uncertainty_sim.py)")
    p.add_argument("--figures-dir", type=Path,
                   default=Path("docs/figures/uncertainty_simulation_large_text_v02"),
...
def main() -> None:
    ...
    print(f"  Reproduce: python scripts/uncertainty_sim_large_text_v02.py "
