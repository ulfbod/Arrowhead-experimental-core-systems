// Live message counts for all consumers across all three transport paths:
//   - AMQP consumers (consumer-1/2/3): /stats + RabbitMQ queue rates
//   - Kafka analytics consumer: /stats (SSE path via kafka-authz)
//   - REST consumer: /stats (REST path via rest-authz)

import { usePolling } from '../hooks/usePolling'
import { fetchConsumerStats, fetchQueues, fetchAnalyticsStats, fetchRestConsumerStats } from '../api'
...
function RestConsumerCard({ intervalMs }: { intervalMs: number }) {
  const fetcher = (signal: AbortSignal) => fetchRestConsumerStats(signal)
  const { data: stats, error } = usePolling<RestConsumerStats>(fetcher, intervalMs)

  return (
    <div style={{ ...s.card, borderColor: '#fde68a' }}>
      <div style={s.cardTitle}>REST Consumer</div>
      <div style={{ ...s.transport, color: '#92400e' }}>REST / HTTP (via rest-authz)</div>
      {error
        ? <div style={s.err}>unavailable</div>
        : (
          <>
...
export function ConsumerStatsPanel() {
  ...
  return (
    <section style={s.section}>
      ...
      <p style={s.hint}>Message counts and delivery rates across all three transports, refreshed every 3 s.</p>
      <div style={s.grid}>
        ...
        <RestConsumerCard intervalMs={3_000} />
      </div>
    </section>
  )
}
