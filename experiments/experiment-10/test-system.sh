#!/usr/bin/env bash
# test-system.sh — system test for experiment-10 running under Docker.
#
# UC3 Lawn Mowing as a Service: clean PAP/PIP/PDP access-control architecture.
# policy-sync removed; PAP pushes XACML policies to AuthzForce immediately.
#
# Run from experiments/experiment-10/ with the stack already up:
#
...
smoke_http "profile-ca /health (HTTP)"           http://localhost:8287/health
smoke_http "AuthzForce /health"                   http://localhost:8396/health
smoke_http "PAP /health"                          http://localhost:9305/health
smoke_http "PIP /health"                          http://localhost:9306/health
smoke_http "kafka-authz /health"                  http://localhost:9301/health
smoke_http "pki-rest-authz /health (HTTP)"        http://localhost:9309/health
smoke_http "portal-cloud-ml /health (HTTP)"       http://localhost:9307/health

# ── Section 1: profile-ca PKI endpoints ──────────────────────────────────────
echo
echo "=== 1. profile-ca (localhost:8287) ==="

check_eq "GET /health → 200"  "200" "$(http_code http://localhost:8287/health)"

ca_info=$(http_body http://localhost:8287/ca/info)
...
on_resp=$(http_body -X POST http://localhost:8287/bootstrap/onboarding-cert \
...
CA_FILE=/tmp/exp10-ca.crt
ON_CRT=/tmp/exp10-on.crt ; ON_KEY=/tmp/exp10-on.key
DE_CRT=/tmp/exp10-de.crt ; DE_KEY=/tmp/exp10-de.key
SY_CRT=/tmp/exp10-sy.crt ; SY_KEY=/tmp/exp10-sy.key

_ca_pem=$(curl -s http://localhost:8287/ca/info \
...
_on_resp=$(curl -s -X POST http://localhost:8287/bootstrap/onboarding-cert \
...
# ── Section 4: PAP — Policy Administration Point ─────────────────────────────
echo
echo "=== 4. PAP /status and /policies (localhost:9305) ==="

check_eq "PAP GET /health → 200" "200" "$(http_code http://localhost:9305/health)"

pap_status=$(http_body http://localhost:9305/status)
echo "  raw: $pap_status"
...
assert_json_value "PAP /status → domainExternalId correct" "domainExternalId" "arrowhead-exp10" "$pap_status"
assert_json_gt    "PAP /status → policies ≥ 1"            "policies"          0                "$pap_status"

pap_list=$(http_body http://localhost:9305/policies)
echo "  policies count: $(echo "$pap_list" | grep -oE '"count":[0-9]+' || echo 'N/A')"
assert_json_gt "PAP /policies → count ≥ 1" "count" 0 "$pap_list"

# ── Section 5: PIP — Policy Information Point ─────────────────────────────────
echo
echo "=== 5. PIP /status and /subjects (localhost:9306) ==="

check_eq "PIP GET /health → 200" "200" "$(http_code http://localhost:9306/health)"

pip_status=$(http_body http://localhost:9306/status)
echo "  raw: $pip_status"
assert_json_field "PIP /status → subjects field" "subjects" "$pip_status"

pip_subjects=$(http_body http://localhost:9306/subjects)
echo "  subjects count: $(echo "$pip_subjects" | grep -oE '"count":[0-9]+' || echo 'N/A')"
assert_json_gt "PIP /subjects → count ≥ 1" "count" 0 "$pip_subjects"

pip_attr=$(http_body http://localhost:9306/attributes/service-partner-1)
echo "  service-partner-1 attrs: $pip_attr"
assert_json_value "PIP /attributes/service-partner-1 → certLevel=sy" "certLevel" "sy" "$pip_attr"
assert_json_value "PIP /attributes/service-partner-1 → valid=true"   "valid"     "true" "$pip_attr"

# ── Section 6: AuthzForce ─────────────────────────────────────────────────────
echo
echo "=== 6. AuthzForce (localhost:8396) ==="

check_eq "GET /health → 200"               "200" "$(http_code http://localhost:8396/health)"
check_eq "GET /authzforce-ce/domains → 200" "200" "$(http_code http://localhost:8396/authzforce-ce/domains)"

# ── Section 7: kafka-authz authorization ─────────────────────────────────────
echo
echo "=== 7. kafka-authz /auth/check (localhost:9301) ==="

body=$(http_body -X POST http://localhost:9301/auth/check \
...
echo
echo "=== 8. pki-rest-authz /auth/check (HTTP localhost:9309) ==="

body=$(http_body -X POST http://localhost:9309/auth/check \
  -H 'Content-Type: application/json' \
...
echo
echo "=== 9. portal-cloud-ml (localhost:9307) ==="

check_eq "GET /health → 200" "200" "$(http_code http://localhost:9307/health)"
pm_stats=$(http_body http://localhost:9307/stats)
echo "  raw: $pm_stats"
...
PROBE_SY_CRT=/tmp/exp10-probe-sy.crt
PROBE_SY_KEY=/tmp/exp10-probe-sy.key

_p_on=$(curl -s -X POST http://localhost:8287/bootstrap/onboarding-cert \
  -H 'Content-Type: application/json' -d '{"systemName":"test-probe"}')
...
if [ -z "$_p_on_crt" ]; then
  ...
  echo "$_p_on_crt" > /tmp/exp10-probe-on.crt
  echo "$_p_on_key" > /tmp/exp10-probe-on.key

  _p_de=$(curl -s -X POST \
    --cert /tmp/exp10-probe-on.crt --key /tmp/exp10-probe-on.key \
  ...
  echo "$_p_de_crt" > /tmp/exp10-probe-de.crt
  echo "$_p_de_key" > /tmp/exp10-probe-de.key

  _p_sy=$(curl -s -X POST \
    --cert /tmp/exp10-probe-de.crt --key /tmp/exp10-probe-de.key \
    --cacert "$CA_FILE" --resolve "profile-ca:8088:127.0.0.1" \
    https://profile-ca:8088/ca/system-cert \
    -H 'Content-Type: application/json' -d '{"systemName":"test-probe"}' 2>/dev/null || echo "")
...
echo
echo "=== 11. service-partner-1 /stats (localhost:9311) ==="

sp_stats=""
for i in $(seq 1 12); do
  sp_stats=$(http_body http://localhost:9311/stats 2>/dev/null || echo "{}")
...
pm_stats=""
for i in $(seq 1 12); do
  pm_stats=$(http_body http://localhost:9307/stats 2>/dev/null || echo "{}")
...
echo
echo "=== 13. pki-rest-authz /status (localhost:9309) ==="

ra_status=$(http_body http://localhost:9309/status)
echo "  raw: $ra_status"
...
sse=$(timeout 4 curl -sN \
  "http://localhost:9301/stream/test-probe?service=telemetry" 2>/dev/null || true)
...
# ── Section 15: Instant revocation via PAP (no sync delay) ───────────────────
echo
echo "=== 15. Instant revocation: PAP delete → immediate AuthzForce effect ==="

pol_list=$(curl -s http://localhost:9305/policies)
...
if [ -z "$pol_id" ]; then
  fail "Revocation: find PAP policy for service-partner-1" "policy" "not found"
  echo "  Skipping instant-revocation test."
else
  pass "Revocation: found PAP policy id=$pol_id"

  before=$(http_body -X POST http://localhost:9309/auth/check \
    -H 'Content-Type: application/json' \
    -d '{"consumer":"service-partner-1","service":"telemetry-rest"}')
  assert_json_value "service-partner-1 → Permit before revocation" "decision" "Permit" "$before"

  del_code=$(curl -s -o /dev/null -w "%{http_code}" \
    -X DELETE "http://localhost:9305/policies/$pol_id"; echo -n "")
  check_eq "PAP DELETE /policies/$pol_id → 204" "204" "$del_code"

  # No sleep — PAP pushes to AuthzForce synchronously on Delete.
  after=$(http_body -X POST http://localhost:9309/auth/check \
    -H 'Content-Type: application/json' \
    -d '{"consumer":"service-partner-1","service":"telemetry-rest"}')
  echo "  auth check immediately after PAP delete: $after"
  assert_json_value "service-partner-1 → Deny immediately after PAP delete" "decision" "Deny" "$after"

  # Re-grant via PAP
  regrant=$(curl -s -X POST http://localhost:9305/policies \
    -H 'Content-Type: application/json' \
  ...
  after2=$(http_body -X POST http://localhost:9309/auth/check \
    -H 'Content-Type: application/json' \
    -d '{"consumer":"service-partner-1","service":"telemetry-rest"}')
  echo "  auth check after re-grant: $after2"
  assert_json_value "service-partner-1 → Permit immediately after re-grant" "decision" "Permit" "$after2"
fi

# ── Section 16: PAP CRUD — create, get, delete ───────────────────────────────
echo
echo "=== 16. PAP CRUD (localhost:9305) ==="

new_pol=$(curl -s -X POST http://localhost:9305/policies \
  -H 'Content-Type: application/json' \
...
get_pol=$(http_body http://localhost:9305/policies/"$new_id")
assert_json_value "PAP GET /policies/$new_id → subject" "subject" "test-crud" "$get_pol"

del2=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE http://localhost:9305/policies/"$new_id"; echo -n "")
check_eq "PAP DELETE /policies/$new_id → 204" "204" "$del2"

check_eq "PAP GET after delete → 404" "404" "$(http_code http://localhost:9305/policies/$new_id)"

# ── Section 17: PIP CRUD — register, get attributes, delete ─────────────────
echo
echo "=== 17. PIP CRUD (localhost:9306) ==="

new_sub=$(curl -s -X POST http://localhost:9306/subjects \
  -H 'Content-Type: application/json' \
  -d '{"name":"test-system","certLevel":"sy","valid":true}')
echo "  register: $new_sub"
assert_json_value "PIP register → name"      "name"      "test-system" "$new_sub"
assert_json_value "PIP register → certLevel" "certLevel" "sy"          "$new_sub"

attr=$(http_body http://localhost:9306/attributes/test-system)
assert_json_value "PIP attributes → certLevel" "certLevel" "sy"   "$attr"
assert_json_value "PIP attributes → valid"     "valid"     "true" "$attr"

del3=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE http://localhost:9306/subjects/test-system; echo -n "")
check_eq "PIP DELETE /subjects/test-system → 204" "204" "$del3"

check_eq "PIP GET after delete → 404" "404" "$(http_code http://localhost:9306/attributes/test-system)"
...
if [ -f "$PROBE_SY_CRT" ] && [ -f "$PROBE_SY_KEY" ]; then
  for _svc_port in "serviceregistry:8690:serviceregistry:8490" "authentication:8691:authentication:8491" "consumerauth:8692:consumerauth:8492" "dynamicorch:8693:dynamicorch:8493"; do
...
rm -f "$CA_FILE" "$ON_CRT" "$ON_KEY" "$DE_CRT" "$DE_KEY" "$SY_CRT" "$SY_KEY" \
      ...
      /tmp/exp10-probe-on.crt /tmp/exp10-probe-on.key \
      /tmp/exp10-probe-de.crt /tmp/exp10-probe-de.key 2>/dev/null || true
