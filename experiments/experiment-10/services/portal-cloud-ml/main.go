...
//	AMQP_URL        AMQPS broker URL for direct telemetry subscription (optional)
//	                 e.g. amqps://portal-cloud-ml:consumer-secret@rabbitmq:5671/
package main
...
func portalConfigFromEnv() portalConfig {
	return portalConfig{
		...
		amqpURL:       envOr("AMQP_URL", ""),
	}
}

// acquireWithRetry retries AcquireSystemCert up to maxRetries times with 3 s delay.
func acquireWithRetry(caURL, caTLSURL, systemName string, maxRetries int) (tls.Certificate, *x509.CertPool, error) {
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if attempt < maxRetries {
			...
			return tls.Certificate{}, nil, fmt.Errorf("lifecycle failed after %d attempts: %w", maxRetries, lcErr)
		}
	}
	return tls.Certificate{}, nil, fmt.Errorf("lifecycle: no attempts made")
}
...
func main() {
	...
	// Determine transport label before creating the store.
	transport := "kafka-sse"
	if envOr("AMQP_URL", "") != "" {
		transport = "kafka-sse+amqp"
	}
	...
	ownCert, caPool, err := acquireWithRetry(cfg.caURL, cfg.caTLSURL, cfg.systemName, 10)
	if err != nil {
		log.Fatalf("[portal-cloud-ml] %v", err)
	}
	...
	// Start AMQP subscriber if configured.
	if cfg.amqpURL != "" {
		amqpTLS := &tls.Config{
		...
		go SubscribeAMQP(cfg.amqpURL, amqpTLS, store, done)
	}
