...
WORKDIR /app
COPY experiments/experiment-5/dashboard/package.json ./
RUN npm install
COPY experiments/experiment-5/dashboard/ .
...
COPY experiments/experiment-5/dashboard/nginx.conf /etc/nginx/conf.d/default.conf
EXPOSE 80
