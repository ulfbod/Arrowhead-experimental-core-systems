# Builds the experiment-8 dashboard (React + Vite → nginx).
...
WORKDIR /app
COPY experiments/experiment-8/dashboard/package.json ./
RUN npm install
COPY experiments/experiment-8/dashboard/ .
...
COPY experiments/experiment-8/dashboard/nginx.conf /etc/nginx/conf.d/default.conf
EXPOSE 80
