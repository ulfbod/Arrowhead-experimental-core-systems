...
const SERVICE_OPTIONS = ['telemetry', 'telemetry-rest']
const PROVIDER_MAP: Record<string, string> = {
  'telemetry':      'robot-fleet-tls',
  'telemetry-rest': 'data-provider-tls',
}
...
export function GrantsPanel() {
  ...
  const handleAdd = useCallback(async () => {
    ...
    setAddState('loading')
    setAddError('')
    const provider = PROVIDER_MAP[newService] ?? 'robot-fleet-tls'
    try {
    ...
    <div style={s.wrap}>
      ...
      <section style={s.section}>
        ...
        <div style={s.form}>
          ...
          >
            {SERVICE_OPTIONS.map(svc => <option key={svc} value={svc}>{svc}</option>)}
          </select>
          <span style={s.dim}>→ {PROVIDER_MAP[newService] ?? 'robot-fleet-tls'}</span>
          <button
        ...
        <p style={s.hint}>
          ...
          Consumer identity in the REST path comes from the X.509 system certificate CN (OU=sy).
        </p>
      </section>
    </div>
  )
