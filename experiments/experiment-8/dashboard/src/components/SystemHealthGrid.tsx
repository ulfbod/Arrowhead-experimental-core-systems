...
const SYSTEMS: SystemDef[] = [
  ...
  { id: 'profile-ca',      label: 'ProfileCA (AH5.2)',healthPath: '/api/profile-ca',      layer: 'core' },
  // ── Support ────────────────────────────────────────────────────────────────
  {
    id: 'rabbitmq', label: 'RabbitMQ (AMQPS)', healthPath: '/api/rabbitmq',
    healthFetcher: fetchRabbitMQHealth,
    layer: 'support',
  },
  { id: 'authzforce',  label: 'AuthzForce',    healthPath: '/api/authzforce',  layer: 'support' },
  { id: 'kafka',       label: 'Kafka (SSL)',   healthPath: '/api/kafka-authz', layer: 'support' },
  // ── Experiment ─────────────────────────────────────────────────────────────
  ...
  { id: 'pki-rest-authz',   label: 'PKIRestAuthz(mTLS)',   healthPath: '/api/pki-rest-authz',   layer: 'experiment' },
  { id: 'robot-fleet',      label: 'RobotFleet(TLS)',      healthPath: '/api/robot-fleet',      layer: 'experiment' },
  { id: 'data-provider-tls',label: 'DataProvider(TLS)',    healthPath: '/api/data-provider-tls',layer: 'experiment' },
  ...
  { id: 'analytics-consumer',label: 'Analytics(SSE)',      healthPath: '/api/analytics-consumer',layer: 'experiment' },
  { id: 'pki-consumer',     label: 'PKIConsumer(mTLS)',    healthPath: '/api/pki-consumer',     layer: 'experiment' },
]
...
export function SystemHealthGrid() {
  ...
  return (
    <section>
      ...
      <p style={s.note}>
        ProfileCA is the Arrowhead 5.2 Local Cloud CA with profile enforcement.
        Services labelled (mTLS) require a system certificate (OU=sy) obtained via the full
        on→de→sy lifecycle. Services labelled (TLS) use file-provisioned certs from cert-provisioner.
      </p>
    </section>
  )
}
...
const s: Record<string, React.CSSProperties> = {
  ...
  note:       { fontSize: '0.7rem', color: '#888', marginTop: 8, fontStyle: 'italic' },
}
