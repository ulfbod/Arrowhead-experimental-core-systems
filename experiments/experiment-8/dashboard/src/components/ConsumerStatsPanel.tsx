// Live message counts for all consumers across all three TLS-secured transport paths:
//   - AMQP consumers (consumer-1/2/3): /stats + RabbitMQ queue rates (AMQPS/TLS)
//   - Kafka analytics consumer: /stats (SSE path via kafka-authz, Kafka SSL)
//   - pki-consumer: /stats (mTLS REST path via pki-rest-authz, profile-PKI identity)

import { usePolling } from '../hooks/usePolling'
import { fetchConsumerStats, fetchQueues, fetchAnalyticsStats, fetchPKIConsumerStats } from '../api'
import type { ConsumerStats, AnalyticsStats, PKIConsumerStats, RabbitQueue } from '../types'
...
function AmqpConsumerCard({
  ...
  return (
    <div style={s.card}>
      ...
      <div style={s.transport}>AMQPS / RabbitMQ (TLS)</div>
      {error
...
function AnalyticsConsumerCard({ intervalMs }: { intervalMs: number }) {
  ...
  return (
    <div style={{ ...s.card, borderColor: '#a5f3fc' }}>
      ...
      <div style={{ ...s.transport, color: '#0891b2' }}>Kafka SSL / SSE</div>
      {error
        ? <div style={s.err}>unavailable</div>
        : (
          <>
...
function PKIConsumerCard({ intervalMs }: { intervalMs: number }) {
  const fetcher = (signal: AbortSignal) => fetchPKIConsumerStats(signal)
  const { data: stats, error } = usePolling<PKIConsumerStats>(fetcher, intervalMs)

  return (
    <div style={{ ...s.card, borderColor: '#d8b4fe' }}>
      <div style={s.cardTitle}>pki-consumer</div>
      <div style={{ ...s.transport, color: '#6d28d9' }}>mTLS REST · PKI profile identity (OU=sy)</div>
      {error
        ? <div style={s.err}>unavailable</div>
        : (
          <>
            ...
            <div style={s.stat}>
              <span style={s.statLabel}>denied count</span>
              <span style={s.statValue}>{stats?.deniedCount ?? '…'}</span>
            </div>
            <div style={s.stat}>
              <span style={s.statLabel}>last denied at</span>
              <span style={s.statValue}>{stats?.lastDeniedAt ? formatTime(stats.lastDeniedAt) : '—'}</span>
...
export function ConsumerStatsPanel() {
  ...
  return (
    <section style={s.section}>
      ...
      <p style={s.hint}>
        Message counts and delivery rates across all three TLS-secured transports, refreshed every 3 s.
        pki-consumer uses a system certificate obtained via the full on→de→sy PKI lifecycle.
      </p>
      <div style={s.grid}>
        ...
        <PKIConsumerCard intervalMs={3_000} />
      </div>
    </section>
  )
}
