...
// This enables interactive demonstration of the sync-delay caveat without
// restarting any service.
//
// Experiment-8 note: policy-sync runs on port 9105 (not 9095 as in experiment-7).
...
export function ConfigView() {
  ...
  function updatePolling<K extends keyof DashboardConfig['polling']>(k: K, v: DashboardConfig['polling'][K]) {
    setDraft((d: DashboardConfig) => ({ ...d, polling: { ...d.polling, [k]: v } }))
  }
  ...
  return (
    <div style={s.wrap}>
      ...
      <section style={s.section}>
        ...
        <Field label="Policy sync (ms)" hint="policy-sync /status + kafka-authz /status + pki-rest-authz /status poll rate">
      ...
      <section style={s.section}>
        ...
        <Field label="Show health latency">
          <input
            ...
            onChange={e => setDraft((d: DashboardConfig) => ({ ...d, display: { ...d.display, showHealthLatency: e.target.checked } }))}
          />
        </Field>
      </section>
