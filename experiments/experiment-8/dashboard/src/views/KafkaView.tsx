...
import type { KafkaAuthzStatus, AuthCheckResult } from '../types'
...
function StatusCard({ status }: { status: KafkaAuthzStatus | null }) {
  return (
    <div style={s.card}>
      ...
      <div style={{ ...s.transport }}>Kafka / SSE — PEP · port 9101</div>
      <div style={s.stat}>
    ...
    <div style={s.checkWrap}>
      <div style={s.checkRow}>
        ...
        <input
          ...
          list="known-consumers-kafka"
        />
        <datalist id="known-consumers-kafka">
...
function KafkaDiagram() {
  ...
  │  port 9101                  │
  │                             │
  │  1. POST AuthzForce /pdp    │──► AuthzForce PDP (arrowhead-exp8)
  ...
  └─────────────────────────────┘
        ...
        ▼
  Kafka broker  (arrowhead.telemetry) — SSL/TLS
  ← robot-fleet-tls publishes here (Kafka SSL)
        │
        │  SSE event: data: {...}
        ▼
