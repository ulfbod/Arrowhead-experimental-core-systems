// cert-provisioner for experiment-8.
// Fetches TLS certificates from the profile-ca (using the backward-compatible
// POST /ca/certificate/issue endpoint) and writes them to a shared Docker volume
// so that Kafka and RabbitMQ can use TLS.
//
...
//
//	CA_URL    Base URL of the profile-ca (default: http://profile-ca:8087)
...
func issueCert(caURL, systemName string, client *http.Client, maxAttempts int, retryDelay time.Duration) (issueCertResponse, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		log.Printf("[cert-provisioner] issued cert for %q", systemName)
		return certResp, nil
	}
