...
// testCA is a minimal CA helper for lifecycle tests.
type testCA struct {
...
func newTestCAForLifecycle(t *testing.T) *testCA {
	t.Helper()
	...
	return &testCA{key: key, cert: cert, certPEM: certPEM}
}

func (tc *testCA) issueLeaf(t *testing.T, systemName, profile string) (string, string) {
	t.Helper()
	key, _ := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	tmpl := &x509.Certificate{
...
func (tc *testCA) tlsCert(t *testing.T) tls.Certificate {
	t.Helper()
