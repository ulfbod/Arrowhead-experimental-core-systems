...
restore_all() {
  for name in demo-consumer-1 demo-consumer-2 demo-consumer-3; do
    http_body -X POST http://localhost:8082/consumerauthorization/authorization/grant \
...
grant_id=$(http_body http://localhost:8082/consumerauthorization/authorization/lookup |
