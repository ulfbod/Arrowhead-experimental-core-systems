...
func TestPIPClient_GetAttributes_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/pip/attributes/pki-consumer" {
			http.NotFound(w, r)
			return
		}
	...
	attrs, err := pip.GetAttributes("pki-consumer")
	if err != nil {
...
// TestPIPClient_GetAttributes_NotFound verifies that a 404 returns fail-closed attrs.
