// fleet.go — manages N robot goroutines with dual-publish (AMQP + Kafka).
package main
...
// PublishFn is a function that publishes a message to one or more transports.
type PublishFn func(routingKey string, payload []byte)
...
// basicPayload is the minimal telemetry message.
...
// NewFleet creates and starts a fleet using the broker's Publish method.
func NewFleet(b *broker.Broker, cfg FleetConfig) *Fleet {
	fn := func(key string, payload []byte) {
		if err := b.Publish(key, payload); err != nil {
			log.Printf("[robot-fleet-tls] AMQP publish error: %v", err)
		}
	}
	return NewFleetWithPublisher(b, cfg, fn)
}

// NewFleetWithPublisher creates and starts a fleet with a custom publish callback.
func NewFleetWithPublisher(_ *broker.Broker, cfg FleetConfig, fn PublishFn) *Fleet {
	f := &Fleet{publishFn: fn}
	f.apply(cfg)
	return f
}
...
func (f *Fleet) apply(cfg FleetConfig) {
	...
	for _, rc := range cfg.Robots {
		...
		go runRobot(ctx, f.publishFn, rc, cfg.PayloadHz, cfg.PayloadType, counter)
	}
}
...
func runRobot(
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				log.Printf("[robot-fleet-tls/%s] marshal error: %v", rc.ID, err)
				continue
			}
