// robot-fleet-tls for experiment-13.
//
// Extends experiment-7 robot-fleet-tls by adding mTLS client certificate
//   - RabbitMQ: AMQPS (TLS AMQP with client cert (mTLS) — required by experiment-13 RabbitMQ
...
// the tls.Certificate (for use as a client cert on other connections) and an
// *http.Client that presents the cert in TLS handshakes.
func buildMTLSClient(caURL, systemName string, caPool *x509.CertPool) (tls.Certificate, *http.Client, error) {
	...
	if err != nil {
		return tls.Certificate{}, nil, fmt.Errorf("POST /ca/certificate/issue: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		return tls.Certificate{}, nil, fmt.Errorf("POST /ca/certificate/issue returned %d", resp.StatusCode)
	}
	var issued caIssueCertResponse
	if err := json.NewDecoder(resp.Body).Decode(&issued); err != nil {
		return tls.Certificate{}, nil, fmt.Errorf("decode issue cert response: %w", err)
	}
	cert, err := tls.X509KeyPair([]byte(issued.Certificate), []byte(issued.PrivateKey))
	if err != nil {
		return tls.Certificate{}, nil, fmt.Errorf("parse issued cert/key: %w", err)
	}
	tlsCfg := &tls.Config{
...
func main() {
	...
	// Also retain the issued tls.Certificate for AMQPS mTLS (experiment-13
	// RabbitMQ requires fail_if_no_peer_cert — client cert CN becomes AMQP username).
	log.Printf("[robot-fleet-tls] issuing own certificate from CA")
	var coreClient *http.Client
	var ownCert tls.Certificate
	for attempt := 1; attempt <= 10; attempt++ {
		cert, c, err := buildMTLSClient(caURL, systemName, caPool)
		if err != nil {
			if attempt < 10 {
	...
	// TLS config for AMQPS: mTLS — client cert required by experiment-13 RabbitMQ
	// (fail_if_no_peer_cert=true; cert CN becomes the AMQP username via ssl_cert_login).
	...
	// TLS config for Kafka: mTLS — client cert required by experiment-13 Kafka
