# Builds the experiment-7 dashboard (React + Vite → nginx).
...
WORKDIR /app
COPY experiments/experiment-7/dashboard/package.json ./
RUN npm install
COPY experiments/experiment-7/dashboard/ .
...
COPY experiments/experiment-7/dashboard/nginx.conf /etc/nginx/conf.d/default.conf
EXPOSE 80
