...
//   - cert-consumer: /stats (mTLS REST path via cert-rest-authz)

import { usePolling } from '../hooks/usePolling'
import { fetchConsumerStats, fetchQueues, fetchAnalyticsStats, fetchCertConsumerStats } from '../api'
import type { ConsumerStats, AnalyticsStats, CertConsumerStats, RabbitQueue } from '../types'
...
function CertConsumerCard({ intervalMs }: { intervalMs: number }) {
  const fetcher = (signal: AbortSignal) => fetchCertConsumerStats(signal)
  const { data: stats, error } = usePolling<CertConsumerStats>(fetcher, intervalMs)

  return (
    <div style={{ ...s.card, borderColor: '#d8b4fe' }}>
      <div style={s.cardTitle}>cert-consumer</div>
      <div style={{ ...s.transport, color: '#6d28d9' }}>mTLS REST (identity from cert CN)</div>
      {error
        ? <div style={s.err}>unavailable</div>
        : (
          <>
...
export function ConsumerStatsPanel() {
  ...
  return (
    <section style={s.section}>
      ...
      <p style={s.hint}>Message counts and delivery rates across all three TLS-secured transports, refreshed every 3 s.</p>
      <div style={s.grid}>
        ...
        <CertConsumerCard intervalMs={3_000} />
      </div>
    </section>
  )
}
