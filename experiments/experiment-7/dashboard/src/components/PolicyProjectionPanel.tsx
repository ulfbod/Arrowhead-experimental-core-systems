...
// Experiment-6 adds a third transport (REST) alongside AMQP and Kafka.

import { usePolling } from '../hooks/usePolling'
import { fetchPolicySyncStatus, fetchKafkaAuthzStatus, fetchCertRestAuthzStatus } from '../api'
...
export function PolicyProjectionPanel() {
  ...
  const { data: restStatus } =
    usePolling(fetchCertRestAuthzStatus, 5_000)
    ...
    <div style={s.wrap}>
      ...
      <section style={s.section}>
        ...
        <p style={s.hint}>
          ...
          compiles them into a XACML 3.0 PolicySet, and uploads it to AuthzForce (PAP).
          All three PEPs query the same AuthzForce domain for enforcement decisions (PDP).
        </p>
      </section>
      ...
      <section style={s.section}>
        ...
        <div style={s.transportGrid}>
          ...
          <div style={{ ...s.transportCard, borderColor: '#fde68a' }}>
            <div style={{ ...s.transportTitle, color: '#6d28d9' }}>REST / mTLS</div>
            <div style={s.transportBody}>
              <div style={s.row}>
                <span style={s.rowLabel}>PEP</span>
                <span style={s.rowValue}>cert-rest-authz</span>
              </div>
              <div style={s.row}>
                <span style={s.rowLabel}>protocol</span>
                <span style={s.rowValue}>HTTP reverse proxy</span>
              </div>
              <div style={s.row}>
                <span style={s.rowLabel}>enforcement</span>
                <span style={s.rowValue}>per request (X-Consumer-Name)</span>
              </div>
              <div style={s.row}>
  ...
  policy-sync ──► AuthzForce PDP/PAP  (XACML 3.0 PolicySet)
           ...
           │           │               │
           ▼           ▼               ▼
  topic-auth-xacml  kafka-authz     cert-rest-authz
  (RabbitMQ AMQP)   (Kafka SSE)   (HTTP proxy)
           │           │               │
           ▼           ▼               ▼
...
`.trim()}</pre>
        <p style={s.hint}>
          ...
          A revoked grant continues to produce Permit decisions until policy-sync
          uploads the next PolicySet version to AuthzForce.
        </p>
      </section>
    </div>
  )
