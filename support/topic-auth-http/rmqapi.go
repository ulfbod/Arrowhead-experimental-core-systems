...
// RabbitMQ 3.12+ returns tags as a JSON array.
...
// rmqConnection models a single entry from GET /api/connections.
type rmqConnection struct {
	Name string `json:"name"`
	User string `json:"user"`
}

// listConnections returns all current AMQP connections.
func (c *rmqClient) listConnections() ([]rmqConnection, error) {
	var conns []rmqConnection
	if err := c.doGET("/api/connections", &conns); err != nil {
		return nil, err
	}
	return conns, nil
}

// deleteConnection force-closes the AMQP connection with the given name.
func (c *rmqClient) deleteConnection(name string) error {
	return c.doDELETE("/api/connections/" + url.PathEscape(name))
}
...
func (c *rmqClient) ensureUser(username, password string) error {
	body := rmqUserBody{Password: password, Tags: managedTag}
